...
export const projects: Project[] = [
  {
    ...
    architecture: 'GroundControl reconciles deployments, Docker/Compose, proxy routes and host state locally or over SSH. Remote agents receive access only to approved workloads and actions; redeploys are durable and retry-safe; operators get a real stateful host terminal in the browser; repair is tied to the exact deployed revision.',
    highlights: [
      'Built MCP/OAuth access that lets approved agents inspect or redeploy specific workloads without receiving SSH credentials or unrestricted server access.',
      'Implemented browser-based PTY sessions over Socket.IO and SSH, plus a Docker/nsenter host bridge, giving operators a stateful host shell for live troubleshooting inside the control plane.',
  ...
  },
  {
    ...
    architecture: 'An LLM-guided research pipeline coordinates web and location-provider search, reconciles results into ranked options, and preserves confirmed facts and uncertainty across turns. Saved plans use revisioned state and optimistic concurrency; selected options become helper tasks through retry-safe PostgreSQL transaction boundaries while payment, matching and acceptance remain separate.',
    highlights: [
      'Built a multi-stage research pipeline that turns natural-language travel and activity intent into multi-source search and ranked options while preserving source tracking and confirmed user decisions.',
